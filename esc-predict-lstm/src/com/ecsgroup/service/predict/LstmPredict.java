...
import com.ecsgroup.service.lstm.LossLayer;
import com.ecsgroup.service.lstm.Lstm;
import com.ecsgroup.service.lstm.LstmNode;
import com.ecsgroup.service.lstm.LstmState;
...
public class LstmPredict {
	...
	public static Lstm TrainLstm(double[][] sample, double[] label, int epoch, int hDim, int xDim, double baselr, double baseMinLoss){
		...
		while(loss>minLoss && iter<epoch ){			
			...
			loss = lstm.y_list_is(label, new LossLayer());        //loss值计算函数，同时计算梯度值
			
			//if(iter%10==0){
				lr = baselr * Math.pow(0.993, iter); 
